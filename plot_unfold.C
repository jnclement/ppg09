...
int plot_unfold(int zsel = 0, int cutsel = 0, int radind = 4, int it = 2)
{
  ...
  TH1D* h_corr[2];
  TH1D* h_ufld[2];
  TH1D* h_truth = (TH1D*)unfdatf->Get(("h_truth_"+cuttype[cutsel]+"_"+ztype[zsel]).c_str());
  ...
  for(int i=0; i<2; ++i)
    {
      h_truth->SetMarkerColor(kGreen+1);
      h_truth->SetMarkerSize(1.5);
      h_truth->SetLineColor(kGreen+1);
      h_truth->SetMarkerStyle(i==0?21:20);
      ...
      h_truth->GetYaxis()->SetTitle("Unfolded Counts");
      h_truth->GetXaxis()->SetTitle("Jet p_{T} [GeV]");
    }
 ...
 for(int i=1; i<hibincorr; ++i)
  ...
  leg->AddEntry(h_ufld[0],"Unfolded","p");
  leg->AddEntry(h_truth,"Truth","p");
  ...
  h_ufld[0]->GetYaxis()->SetRangeUser(1e-3,1e6);
  h_ufld[0]->Draw("PE");
